...
as explained in the progress report. The sparse matrix multiplication is fully parallel, so leaving the overhead for thread creation aside, the runtime complexity is 

\begin{equation}
    O_{parSolve}(N, C) = O \left(\frac{N^3}{C}\right) + O(N^2)
\end{equation}
...
The equation solver has weak scalability, . In practice however, there is a significant overhead in creating the threads. This means, that the speedup becomes only notable for problem sizes above 50.
