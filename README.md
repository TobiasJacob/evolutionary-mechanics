...
- The sequential implementations and the basic parallelized implementation are all contained in their own folder. Please, use the following steps:
    1. In order to run on Bridges, due to the dependency on C++17, make sure you run `module load gcc/9.2.0` first
    2. `cd version_to_execute`
    3. `make`
    4. `build/program <N> <Organisms> <Epochs> <Decay>` (recommend 20 100 1000 0.995f)
